...
// Ajuste para que el canvas siempre tenga el tamaño correcto del móvil
...
// ESTO ES PARA QUE FUNCIONE EN MÓVIL AL TOCAR EL CORAZÓN
...
function iniciarCaida() {
    ...
    function caer() {
        ...
        if (posY >= PUNTO_INICIO_ARBOL) {
            ...
            // Ajuste de magnitud para que el árbol quepa en pantallas móviles
            let magnitudBase = window.innerWidth < 600 ? 80 : 110;
...
function bucleAnimacion() {
    ...
    // Ajuste de desplazamiento para móvil (que no se salga tanto de la pantalla)
    let limiteDesplazamiento = window.innerWidth < 600 ? canvas.width * 0.1 : canvas.width * 0.25;

    if (arbolListo && desplazamientoX < limiteDesplazamiento) { 
        desplazamientoX += 2.5; 
